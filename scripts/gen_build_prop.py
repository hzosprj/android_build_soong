...
def parse_args():
  ...
  config["HertzifyDesc"] = config["BuildDesc"]
  config["HertzifyDevice"] = config["DeviceName"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] == "user":
    ...
    print(f"ro.build.display.id?={config['HertzifyDesc']}")
  ...
  print(f"ro.hertzify.device={config['HertzifyDevice']}")
